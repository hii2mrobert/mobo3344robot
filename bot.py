        ...
        if chats_list != []:
            for chat in chats_list:
                ...
                if not m_id in list_message_seened and chat['object_guid'] in admins:
                    ...
                    if text == "آپلود" :
                        bot.sendMessage(chat['object_guid'],'آیدی کانال مورد نظر را با @ وارد کنید.\nمثال:     @sterik_bot')
                    elif text.startswith("@"):
                        id = text.split("@")[1]
                        guid_of_channel = bot.getInfoByUsername(id)['data']['channel']['channel_guid']
                        bot.sendMessage(chat['object_guid'],'کپشن فایل را وارد کنید.\nمثال:   \n fa [کپشن]')
                    elif text.startswith("fa ["):
                        caption = id = text.split("[")[1][:-1]
                        bot.sendMessage(chat['object_guid'],'فرمت و نام فایل را وارد کنید.\nمثال برای فیلم:\n[name.mp4')
                    elif text.startswith("["):
                        new_name = f" {text}"
                        ...
                        bot.sendMessage(chat['object_guid'],'آپلود فایل شروع شد۰.')
                                ...
                                while True:
                                    try:
                                        ...
                                        if a > tweny_five and is_tweny_five == False:
                                            bot.sendMessage(chat['object_guid'],'25%آپلود شده است')
                                            is_tweny_five = True
                                        elif a > fifty and is_fifty == False:
                                            bot.sendMessage(chat['object_guid'],'50% آپلود شده است')
                                            is_fifty = True
                                        elif a > seventy_five and is_seventy_five == False:
                                            bot.sendMessage(chat['object_guid'],'75% آپلود شده است')
                        ...
                        if j != None and type(j) == str:
                            ...
                            mir=str(int(datetime.datetime.now().timestamp() - start_time))
                            bot.sendMessage(chat['object_guid'], 'فایل در '+mir+' ثانیه آپلود شد')
                            print('send file'+mir)
                        else:
